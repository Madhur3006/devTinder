...
userRouter.get("/user/connections", userAuth, async (req, res) => {
  ...
  try {
    ...
    const data = connections
      .map((connection) => connection.fromUserId)
      .filter((user) => !user._id.equals(_id));
    res.json({
...
userRouter.get("/feed", userAuth, async (req, res) => {
  ...
  try {
    ...
    const connectionRequest = await ConnectionRequest.find({
      // for finding all connected users
      $or: [{ toUserId: loggedInUser._id }, { fromUserId: loggedInUser._id }],
    });

    const hideUserConections = new Set(); // for findind all distinct user id's
    connectionRequest.forEach((req) => {
      // set here is same a python as it only allow distinct values
    ...
    const users = await User.find({
      ...
      .skip(skip)
      .limit(limit); // skip and limit are used for pagination
